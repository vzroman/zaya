...
and the value is `{RollbackWrite, RollbackDelete}` — the inverse operations to undo a committed DB within a transaction. Serialised (via sext), keys sort lexicographically by tag, then `db`, then `seq`, then `tref`, so iterating the `{rollback, DB, _, _}` prefix yields all of a DB's rollback entries in **per-node chronological order** (see "Sequence number" below). All rollback entries for a transaction share a single `seq` and are written in one atomic batch. Only the **single-node commit** additionally bundles the `{aborted, TRef}` marker into that initial batch (and bundles it into the post-commit delete batch); multi-node workers write the marker separately when a rollback decision is reached.

**Aborted entries:** `{ {aborted, TRef}, AllParticipatingDBsNodes }` — evidence that a transaction was rolled back. `AllParticipatingDBsNodes` has format `[{DB, [Node, ...]}, ...]` listing every DB and the nodes that hold a replica of it for this transaction. Used by `is_aborted/1` queries during recovery and as state for the GC sweep.

There is **no committed entry**. Absence of `{aborted, TRef}` on every reachable node implies the transaction was committed (see DB Open Flow recovery rules). When the transaction touches at least one persistent DB, the coordinator logs `{aborted}` on its own node before broadcasting `{rollback}` (unless the coordinator participates, in which case the local worker covers it), and each rolling-back worker also logs `{aborted}` on its own node. When the whole transaction is non-persistent, nothing is logged — no recovery will ever look for the marker.
...
3. **Sweep `{aborted, TRef}` entries:**
   ...
   - When a DB's node list becomes empty, drop the `(DB, _)` tuple; when the entry's DB list becomes empty, delete the `{aborted, TRef}` entry.
...
%%   log aborted only (coordinator step 4.1):
%%     commit([{{aborted, TRef}, AllParticipatingDBsNodes}], []).
...
%% Returns {ok, true} if a {aborted, TRef} entry exists locally,
...
-spec is_aborted(TRef :: reference()) ->
...
%% is_aborted(TRef) cluster-wide via ecall:call_all_wait(zaya:all_nodes(), ...)
...
%% touch {aborted, TRef} entries; those are owned by the gen_server's
...
**API / gen_server boundary.** None of the API functions above (`seq/0`, `commit/2`, `is_aborted/1`, `rollback/2`, `purge/1`, `list_pending_transactions/0,1`) call or cast the gen_server. Each operates on the rocksdb log directly via the Ref kept in `persistent_term` (for `seq/0`, an `atomics` ref likewise kept in `persistent_term`). The gen_server's role is confined to: opening/closing the rocksdb instance and installing the Refs in `persistent_term`, the one-shot startup scan that seeds `seq`, and the periodic Aborted Entry Cleanup sweep described above.
...
CommitData has N DBs -> multi DB commit
  1 node  -> single_node_commit (isolated worker process:
               ...
               rollback + {aborted} written/deleted in atomic batches)
...
3. If NeedLog -> allocate one Seq and single-batch write rollback entries + the aborted marker:
     Seq = transaction_log:seq(),
     transaction_log:commit(
         [{#rollback{db=DB, seq=Seq, tref=TRef}, {RW, RD}} || ...]
         ++ [{{aborted, TRef}, [{DB, [node()]} || DB <- Persistent]}],
         []).
4. Commit each DB sequentially:
     try Module:commit(Ref, Write, Delete)
       ...
       If NeedLog -> single-batch delete of both:
         transaction_log:commit(
             [],
             [#rollback{db=DB, seq=Seq, tref=TRef} || ...] ++ [{aborted, TRef}]).
       exit(Error).
...
- `AllParticipatingDBsNodes` — `[{DB, [Node, ...]}, ...]` listing every DB and its replica nodes for this transaction. Embedded verbatim into any `{aborted, TRef}` entry the worker writes.
- `AnyPersistent` — `boolean()`, set by the coordinator to `true` if at least one participating DB (on any node) is persistent. Every `{aborted, TRef}` write — coordinator's 4.1 and workers' 2.2 / 5.2 / 6.3.2 / 6.3.3.1 — is guarded by this flag. When the whole transaction is non-persistent, nothing is logged anywhere: there are no rollback-data entries to write and no cross-node recovery will ever query `is_aborted/1` for this TRef.
...
2. Phase 1 error (catch branch in step 1.3 or anywhere in step 1):
   ...
   2.2. Single batch combining two conditionally-included groups:
          Writes  = [{{aborted, TRef}, AllParticipatingDBsNodes}]   % only if AnyPersistent
...
5. On {rollback, AllWorkers}:
   5.1. Broadcast {aborted, AllWorkers -- [self()]} to peer workers via ecall:send.
   5.2. If AnyPersistent ->
          transaction_log:commit(
              [{{aborted, TRef}, AllParticipatingDBsNodes}], []).
        — log {aborted} BEFORE applying the rollback so recovery on this
   ...
   5.4. If any LOCAL DB is persistent ->
          transaction_log:commit(
              [], [#rollback{db=DB, seq=Seq, tref=TRef} || ...]).
        (delete rollback entries that 1.2 wrote; the {aborted, TRef}
   ...
   6.3. Wait for one of:
        6.3.1. {committed, _} from a peer:
               ...
               * If any LOCAL DB is persistent ->
                   transaction_log:commit(
                       [], [#rollback{db=DB, seq=Seq, tref=TRef} || ...]).
               * Exit normal.
        6.3.2. {aborted, _} from a peer:
               * Broadcast {aborted, AllWorkers -- [self()]} via ecall:send.
               * If AnyPersistent ->
                   transaction_log:commit(
                       [{{aborted, TRef}, AllParticipatingDBsNodes}], []).
        ...
        6.3.4. `after Timer` (configurable, default e.g. 100 ms): poll each silent
               ...
               group — with ecall:call:
                   zaya_transaction_log:is_aborted(TRef).
               * If any reply is {ok, true}: treat as if a {aborted}
                 broadcast had arrived from that node — run the step 6.3.2
                 actions (broadcast {aborted}, log {aborted}, apply
               ...
               * If every reply is {ok, false} or {error, timeout} or an RPC
                 ...
                 step 2.2 — its local {aborted} marker is detected without
                 a broadcast.
```

**Two guards (implementation note):** the rollback-data and the `{aborted, TRef}` writes use different guards — mirror both as inline comments at each write site in the code.
...
- **`{aborted, TRef}` marker writes** (coordinator 4.1; worker 2.2 / 5.2 / 6.3.2 / 6.3.3.1) are guarded by `AnyPersistent` — true iff **any** participating DB on any node is persistent. A worker with only non-persistent local DBs still contributes to cluster-wide evidence when the transaction touches a remote persistent DB; but if the whole transaction is non-persistent, nobody will ever query `is_aborted/1` for this TRef.

**pg semantics (per the consensus rule):** A worker joins `pg:join(zaya_transaction, TRef)` exactly when it enters step 6. A worker that has decided (steps 4 or 5) is never in the group — either it already broadcast `{committed}` / `{aborted}` or it is about to. So an alive peer that is not in the group is treated as "a broadcast is coming; keep waiting". Step 6.3.3 fires only when every peer is either in the group or has its node down. Step 6.3.4's timer poll covers the one case pg can't distinguish: a peer that phase-1-failed, ran step 2.2 (wrote `{aborted}` locally) and exited without ever joining pg — from the other workers' perspective it looks alive-and-silent; the poll finds the disk evidence directly.

### Log vs. No-Log Decision

The decision applies to both kinds of log entry (`#rollback{}` data entries and the `{aborted, TRef}` marker).

- **All DBs non-persistent** (`AnyPersistent = false`): `prepare_rollback` for all, keep rollback data in memory only — no `#rollback{}` entries and no `{aborted, TRef}` marker. If a node crashes non-persistent data is lost anyway, and no recovery query will ever look for the marker.
- **Any persistent DB** (`AnyPersistent = true`): `prepare_rollback` for all; each worker logs `#rollback{}` entries for its local persistent DBs (step 1.2), and any worker that reaches a rollback decision logs the cluster-wide `{aborted, TRef}` marker on its node. The coordinator also logs the marker on its own node iff it is not a participant (step 4.1).
...
2. If recovered (copied) DB -> zaya_transaction_log:purge(DB).
   ...
   Recovery rule order (first match wins):
     ...
     R3. The coordinator was unreachable (RPC error or is_aborted returned
   ...
   (R1 deliberately wins over R2: a peer with {aborted, TRef} is
...
| Repo | Branch | Changes |
|------|--------|---------|
| zaya | `pool` (exists) | Refactor `zaya_transaction.erl`: per-worker (not per-DB) confirmation model, new coordinator/worker FSMs, `{aborted}`-only logging, peer-to-peer broadcast on coordinator DOWN via `ecall:send`. Add `zaya_transaction_log` gen_server exposing `seq/0`, `commit/2`, `is_aborted/1`, `rollback/2`, `purge/1`, `list_pending_transactions/0` (none of which call/cast the gen_server); gen_server owns only lifecycle, the startup `seq`-seed scan, and the periodic GC sweep (covers active-TRef inhibit, removed DBs, decommissioned nodes). Update `zaya_sup` to start the log process. Update `zaya_db_srv` open flow with the recovery rule order, `PENDING_TRANSACTIONS` env handling, and the "available only after scan" guarantee; DB removal relies on the GC sweep rather than `purge`. Add `zaya:list_pending_transactions/0`. |
...
- **`{aborted}`-only logging; commit is the default:** The log records rollbacks, never commits. Absence of `{aborted, TRef}` on every reachable node implies committed (R2). This eliminates the `{committed}` log-entry GC complexity at the cost of one assumption: cluster-wide reachability of *some* node with evidence. When the transaction touches any persistent DB, the marker is written N-way redundantly — by the coordinator on its node (if non-participant) plus every rolling-back worker on its own node — so even with cascading failures the evidence survives. Fully non-persistent transactions skip the marker entirely (no recovery will ever query for it).
- **R1 wins over R2 (peer-evidence beats coordinator-not-found):** If a phase-1-failing worker logs `{aborted}` and exits before the coordinator finishes its own `{aborted}` write, recovery on a third node could see "coordinator not_found" + "peer aborted" simultaneously. The rule order `R1 > R2` accepts the rollback in this case (per concern C2). The reverse ordering would risk treating a rolled-back transaction as committed.
- **Coordinator-non-participant overhead:** When the coordinator's node has no participating DBs but the transaction has at least one persistent DB, step 4.1 writes a `{aborted}` entry on a node that otherwise holds no data for the transaction. Intentional: the coordinator is the source of truth and `node(TRef)` is the canonical lookup. Cost is one fsync per rollback only; the happy path never logs on the coordinator, and fully non-persistent transactions never log anywhere.
- **Cluster-wide recovery query:** `is_aborted/1` is invoked on every known node, not just DB replicas, so any node with evidence is reached. Non-participants harmlessly answer `{ok, false}`. One `ecall:call_all_wait` per stuck TRef during a DB's open scan (no batching of TRefs into a single call — per F15 resolution).
- **Stuck recovery via env override:** When the coordinator is unreachable AND no peer has `{aborted}` evidence, recovery loops on R3 until either evidence arrives or `PENDING_TRANSACTIONS` is set. The env is global per boot; operators inspect candidates via `zaya:list_pending_transactions/0`. Env stays set across restarts until the operator clears it.
- **O(N²) peer broadcasts:** Workers broadcast `{committed}` / `{aborted}` to all peers in steps 4.2.2, 5.1, 6.3.1, and 6.3.2. Acceptable for typical cluster sizes; revisit if N grows large. Erlang messages are cheap and rollback is the unhappy path.
- **Worker FSM test surface:** Six top-level worker states with nested sub-states yield a non-trivial test matrix (phase1-fail, commit2-then-DOWN-normal, commit2-then-DOWN-error, rollback-received, coordinator-DOWN-then-pg-consensus-rollback, coordinator-DOWN-then-peer-tells-committed, coordinator-DOWN-then-peer-tells-aborted, coordinator-DOWN-with-silent-peer-then-timer-poll). A dedicated FSM test suite is planned during implementation; not a design issue.
- **Coordinator 4.1 skip race:** When the coordinator participates, step 4.1 skips the `{aborted}` write and relies on the local worker's step 5.2. Between the coordinator's decision and the local worker's step 5.2 completion, `node(TRef)`'s log briefly lacks the marker. A concurrent recovery elsewhere that queries `is_aborted/1` on `node(TRef)` during that window would see `{ok, false}` and fall to R2. The window is microseconds to single-digit milliseconds versus the seconds-to-minutes of any recovery reboot path, so the race is not exploitable in practice. Accepted.
